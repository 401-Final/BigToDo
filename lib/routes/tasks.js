...
router
  ...
  .post('/', bodyParser, (req, res, next) => {
    ...
    Context.findOne({name: req.body.contextId, userId: req.user.id})
      .then(context => {
        return context;
      })
      .then(context => {
        Project.findOne({description: req.body.projectId, userId: req.user.id})
          .then(project => {
      ...
      // .then(savedTask => {console.log('saved', savedTask); res.send(savedTask);})
      // .catch(next);
  })
