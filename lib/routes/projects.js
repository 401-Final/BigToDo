...
.delete('/:id', (req, res, next) => {
  ...
  Project.findOneAndRemove({ userId: req.user.id, _id: req.params.id })
    .then(deleted => {
      // issue with sending response before other actions happen is that rest of operation may have failed
      res.send(deleted);
...
.post('/', bodyParser, (req, res, next) => {
  ...
  // .description is the parentId? That makes no sense
  Project.findOne({description: req.body.parentId})
      .select('_id').lean()
      .then(project => {
        if(project) proj.parentId = project._id;
        return proj.save();
      })
